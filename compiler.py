...
def create_dark_fantasy_gauge(w, duration):
    ...
    def make_frame(t):
        ...
        if progress > 0:
            ...
            draw.rectangle([4, 4, 4 + fill_w, gauge_h-40], fill=(255, 60, 60, 100))
        return np.array(img)
...
def _make_text_clip_exact(text, fsize, color, duration, font_path, stroke_w=4):
    ...
    return ImageClip(np.array(img)).with_duration(duration), img_w
...
def compile_video(project_name: str, instance_id: str) -> str:
    ...
    NAME_FSIZE = 50 
    Y_ZONE_NAMES = int(h * 0.45) 
    Y_ZONE_GAUGE = int(h * 0.65) 
    MARGIN_X = int(w * 0.10)
    
    font_p = "assets/Minecraft.ttf"
    lbl_l_exact, tw_l = _make_text_clip_exact(name_l, NAME_FSIZE, "white", 99, font_p, stroke_w=3)
    lbl_r_exact, tw_r = _make_text_clip_exact(name_r, NAME_FSIZE, "white", 99, font_p, stroke_w=3)
    
    POS_L = (MARGIN_X, Y_ZONE_NAMES - 25)
    POS_R = (w - MARGIN_X - tw_r, Y_ZONE_NAMES - 25)
    ...
    if os.path.exists(paths["image"]):
        ...
        draw.text((POS_L[0], POS_L[1]+25), name_l, font=font_pix, fill="white", stroke_width=3, stroke_fill="black")
        draw.text((POS_R[0], POS_R[1]+25), name_r, font=font_pix, fill="white", stroke_width=3, stroke_fill="black")
    ...
    if os.path.exists(paths["narrator"]):
        ...
        # --- PHASE 4 : CHOIX FINAL (Timer accéléré 0.7s) ---
        T_STEP = 0.7 
        CHOICE_DUR = T_STEP * 3 # Total 2.1s
        choice_bg = blur_bg_with_names.with_duration(CHOICE_DUR)
        
        # Timer circulaire au centre absolu
        timer_size = 230
        ...
        # Jauge rouge
        dark_gauge = create_dark_fantasy_gauge(w, CHOICE_DUR).with_position(("center", Y_ZONE_GAUGE - 30))
